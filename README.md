# vr-kanji-spot-the-difference-001

【VRゲーム】漢字間違い探し【001】木性、貫索・石門の部屋。

（A-Frameで作成）

![サムネイル画像](./docs/thumbnail.png)

## 💡 制作のきっかけ (Idea)

- もともと脳トレにハマっていた。
- 漢字間違い探しを作っていた。
- ちょうどA-Frameを学びだした
- 組み合わせたら面白いものが出来るのでは？

## あらすじ

国境の長いトンネルを抜けると
漢字間違い探しであった。

国境の長いトンネルを抜けると雪国であった
川端康成 / 小説『雪国』より

キューブ（Cube）
監督 / ヴィンチェンゾ・ナタリ

## 🎮 遊んでみる・見てみる (Play & Watch)

👉 **[🎮 今すぐブラウザで遊ぶ (Play Demo)](https://gyosei-yuki.github.io/vr-kanji-spot-the-difference-001/)**

※ スマホでもPCでも遊べます！

👉 **[🎥 デモプレイ動画を見る (ニコニコ動画)](https://www.nicovideo.jp/watch/sm45902453)**

## 概要 (Overview)

- タイムアタック🕐
- トンネルを抜け、部屋の壁（前後左右+天井）に現れる漢字の中から「間違い」を視線（カーソル）で探してクリック👆
- 正解すると次のステージへのゲートが開きます。
- 全4ステージをクリアしてゴールを目指せ👍
- オーソドックスなWASDコントローラー🎮

## 使用技術 (Tech Stack)

- HTML / JavaScript
- A-Frame (WebXRフレームワーク)

## 制作者 (Author)

**渋谷佑生 (Yuki Shibuya)**

- 岩手県で活動する「表現する実務家（行政書士）」
- 音楽配信したり
- 算命学のアプリを配信したり

あまり、行政書士っぽくない活動をしています。
